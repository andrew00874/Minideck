...
자주 쓰는 앱·폴더·웹사이트를 하나의 다이얼에 모아 두세요. 노브를 돌리면 아이콘 사이를 이동하고, 누르면 선택한 항목을 실행합니다. 세 키로 등록한 앱, 열린 창, 선택한 글의 검색·번역 모드를 고릅니다. 미니키보드가 없어도 마우스와 일반 키보드로 먼저 사용해 볼 수 있습니다.
...
| 세 가지 다이얼 모드 | 1번: 등록한 앱, 2번: 열린 창, 3번: 선택한 글 검색·번역. |
...
**0.2.0-beta.1 업데이트:** 세 가지 다이얼 모드와 서비스 SVG 아이콘을 제공합니다. 기존 사용자는 실행 파일 두 개를 교체하고 Chrome에서 Browser Link 1.1.0을 새로고침하세요.

| 언어 | 다운로드 |
| --- | --- |
| 한국어 | [MiniDeck 0.2.0-beta.1 — 한국어판 ZIP](https://github.com/andrew00874/Minideck/releases/download/v0.2.0-beta.1/MiniDeck-0.2.0-beta.1-win-ko.zip) |
| English | [MiniDeck 0.2.0-beta.1 — 영문판 ZIP](https://github.com/andrew00874/Minideck/releases/download/v0.2.0-beta.1/MiniDeck-0.2.0-beta.1-win-en.zip) |
...
5. 아래 표에 맞춰 키보드 출력을 설정합니다. **1 / 2 / 3번**은 **등록한 앱 / 열린 창 / 검색·번역**입니다. 기존 MiniDeck 연동 매핑은 다시 저장할 필요가 없습니다.

한국어판과 영문판은 설정을 공유하므로 하나씩 실행하세요. 언어판을 바꿔도 직접 지정한 앱 이름과 아이콘은 유지됩니다.

## 1. 앱 등록과 모드 선택

![한국어 설정창: 앱 목록, 편집·순서 변경 버튼, 세 모드 버튼, 다이얼 시험 버튼과 크기 슬라이더](docs/images/settings-ko.png)

왼쪽 목록의 순서가 다이얼의 순서입니다. 항목을 선택하면 이름·실행 대상·아이콘을 편집할 수 있습니다. **위로 / 아래로**로 다이얼 순서를 바꿉니다. **삭제**는 런처 목록에서만 제거하며 실제 프로그램을 지우지 않습니다.

**1 등록한 앱**은 실행 목록, **2 열린 창**은 현재 열린 창 전환, **3 검색·번역**은 아래의 선택한 글 작업입니다. 2번은 프로그램을 새로 실행하지 않습니다. **선택 앱 실행**으로 앱 실행을 시험할 수 있습니다.
...
| 조작 | 동작 |
| --- | --- |
| 노브 왼쪽·오른쪽 회전 | 선택 이동. 닫혀 있으면 현재 모드를 열며 창·검색 모드는 목록 또는 선택한 글을 다시 가져옵니다. |
...
자세한 내용은 [확장 설치 안내](browser-extension/README.ko.md)를 참고하세요. 확장은 탭 정보와 로컬 연결을 사용하며 페이지 본문·쿠키·비밀번호를 수집하지 않습니다. 검색·번역 모드에서 엔진을 확정하면 선택한 글을 담은 해당 서비스의 결과 주소로 이동합니다. Chrome 웹 스토어 배포가 아닌 압축해제 확장 방식입니다. **Windows의 실제 Chrome에서 기존 탭 전환과 창을 맨 앞으로 가져오는 동작을 확인했습니다. 다른 브라우저 사용 시나리오는 추가 검증이 필요합니다.**

## 선택한 글 검색·번역

![엔진과 선택한 원문을 보여주는 검색·번역 다이얼](docs/images/text-ko.png)

1. 사용 중인 앱에서 글을 선택합니다.
2. **3번 키**를 누르고 손을 뗍니다. 다이얼을 띄우기 전에 선택한 글을 가져옵니다.
3. 노브로 **Google 검색 / 네이버 검색 / Bing 검색 / Google 번역**을 고릅니다. 다이얼 아래에서 가져온 원문을 확인하세요.
4. 노브를 누르면 해당 서비스로 전달하고 결과를 표시합니다. **Esc·다른 모드 선택·자동 닫힘**은 전송 없이 취소합니다.

API 키는 필요 없습니다. 설정의 **검색·번역 엔진**에서 이름과 `{text}`가 들어간 HTTP(S) 결과 주소를 추가·수정할 수 있습니다. 다른 항목을 편집하기 전에 **항목 적용**, 마지막에 **저장**을 누르세요. Google 번역은 한국어판에서 한국어, 영문판에서 영어가 기본 목표 언어이며 기존 설정의 언어는 유지합니다.

**Browser Link 1.1.0 업데이트:** 앱·호스트 파일과 확장 폴더를 함께 업데이트하고 `chrome://extensions`에서 MiniDeck 확장을 새로고침하세요. 검색·번역은 엔진별 전용 결과 탭에서 검색어를 갱신합니다. 일반 웹사이트 항목은 기존 페이지를 유지합니다. Chrome이나 확장을 다시 시작하면 결과 탭 추적이 초기화되어 새 결과 탭을 만듭니다.

원문은 메모리에만 두고 다이얼을 닫으면 비우며, 설정·로그에는 저장하지 않습니다. 확정한 결과 주소는 브라우저 방문 기록에 남을 수 있고 선택한 서비스에 전달됩니다. Windows UI Automation으로 먼저 선택 영역을 읽고, 지원하지 않는 앱에서는 Ctrl+C와 클립보드 복원을 사용합니다. 일부 편집기는 선택이 없을 때 한 줄을 복사하므로 미리보기를 확인하세요. 빈 글·읽을 수 없는 글·너무 긴 글은 거부합니다(최대 2,000자, 인코딩된 주소 길이도 제한). 관리자 권한·보호된 앱은 읽기를 막을 수 있습니다. 연결된 기기에서 글 선택 → 3번 키 → 엔진 선택 → 노브 누름 → 결과 페이지까지 열리는 흐름은 사용자가 확인했습니다. 다른 앱과 관리자 권한 창의 호환성은 추가 검증이 필요합니다.
...
두 언어의 자체 테스트를 통과했고 영문 화면을 검토했습니다. 자동 검사는 설정 이관·선택 영역 읽기·확정과 취소·닫힌 창 판별·전역 단축키·다이얼 이동과 크기·긴 이름·아이콘 가져오기·온보드 패킷·번역 누락·브라우저 탭 선택·로컬 메시지 연결을 다룹니다.
...
최종 확인: 2026-09-18 · 버전: 0.2.0-beta.1
