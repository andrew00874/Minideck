...
Keep your favorite apps, folders and websites on one dial. Turn the knob to move between icons, then press to launch the selected item. Three physical keys can open your most-used items directly. You can also try the interface with a mouse or an ordinary keyboard.
...
| Three direct shortcuts | Assign a different item to each of the three keys. |
...
5. Assign items to **Key 1**, **Key 2** and **Key 3**, then configure your macro pad as shown below.

Both language editions share settings. Run only one at a time; switching editions preserves your own app names and icons.

## 1. Add apps and assign the three keys

![English settings window: app list, edit and reorder controls, three key assignments, dial controls and size slider](docs/images/settings-en.png)

The left list is the dial's order. Select an item to edit its name, target or icon. **Move up / Move down** changes the order; key assignments remain attached to the same items. **Remove** removes the launcher entry, not the installed app.

Use the three dropdowns to choose what each physical key opens. **Open selected** tests a launch without using the macro pad.
...
| Action | Result |
| --- | --- |
| Turn left or right | Open the dial if hidden and move the selection. |
